...
const float ROAD_SPEED = 0.03f; // Slower speed in pixels per frame
...
EnemyCar::EnemyCar(SDL_Renderer* renderer, const std::string& filePath, int x, int y)
    : renderer(renderer), speed(0.1f), accumulatedMovement(0.0f) {
    texture = loadTexture(filePath);
    if (texture == NULL) {
...
int main(int argc, char* argv[]) {
    ...
    Car car(renderer, "./assests/car80.bmp", WINDOW_WIDTH / 2, WINDOW_HEIGHT - 150);

    std::vector<EnemyCar> enemyCars;
    std::srand(static_cast<unsigned>(std::time(0)));
    for (int i = 0; i < 2; ++i) {
        int x =  (std::rand() % (900 ));
        enemyCars.emplace_back(renderer, "./assests/bc.bmp", x, -(std::rand() % WINDOW_HEIGHT));
        int y = (std::rand() % (500 ));
        enemyCars.emplace_back(renderer, "./assests/download__1_-removebg-preview.bmp", y, -(std::rand() % WINDOW_HEIGHT));
        int z = (std::rand() % (300));
        enemyCars.emplace_back(renderer, "./assests/abc-removebg-preview.bmp", z, -(std::rand() % WINDOW_HEIGHT));
        int a =  (std::rand() % (400));
        enemyCars.emplace_back(renderer, "./assests/df-removebg-preview.bmp", a, -(std::rand() % WINDOW_HEIGHT));
       // int b = 210 + (std::rand() % (630 - 210));
       // enemyCars.emplace_back(renderer, "./assests/bc.bmp", b, -(std::rand() % WINDOW_HEIGHT));
